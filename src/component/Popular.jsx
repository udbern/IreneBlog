  ...
  return (
    <section className='my-20 max-w-7xl mx-auto '>
      <div className='flex item-center justify-between  font-Montaga mb-4 '>
        <div>
          <h1 className='uppercase font-bold text-xl px-5'>latest post</h1>
        </div>
        <div>
          <Link to="/blog" className=' mr-5 text-white bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 py-2 px-4 rounded shadow test-gray-800 tracking-wide hover:opacity-75 transition-all duration-200 md:w-auto'  >view all post</Link>
        </div>
      </div>

      {!stories ? <h2>Loading...</h2> : <>{stories[0] && <Link to={`/blog/${stories[0].slug.current}`}>
        <div className=' hidden md:block   overflow-hidden mb-8 shadow-md shadow-indigo-800  '>
            ...
            <div className='absolute bottom-0 left-0  w-full h-full   justify-center items-center 
             bg-blue-600/30 backdrop-brightness-75  '>
              <div className='absolute bottom-8 left-8'>
                <h1 className='capitalize  text-2xl lg:text-3xl mb-6 text-white  '>{stories[0].title}</h1>
                <p className='text-gray-100 text-md mb-8 md:w-1/2 '>{`${stories[0].body[0].children[0].text.substring(0, 50)}...`}</p>
      ...
      <section className=' grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 px-5   '>
        {stories.map((story) => (
          <Link to={`/blog/${story.slug.current}`} key={story.slug.current} >
            <div className='border border-gray-500 rounded-lg overflow-hidden  shadow-md shadow-indigo-800 h-96 min-h-96 '>
              ...
              <div className='p-4'>
                ...
                <h2 className='capitalize text-lg font-bold '>{story.title}</h2>
